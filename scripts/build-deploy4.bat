...
echo Usage: build-deploy4.bat [build^|package^|deploy]
exit /b 1
...
:MODE_PACKAGE
call "%~dp0build-deploy4.bat" build

echo [3/4] Package deploy artifacts...
...
echo 1) Start backend (MySQL and Redis must be available):>>"%DIST_DIR%\DEPLOY_HINT.txt"
...
echo - Next.js standalone default port is 3000 (unless overridden).>>"%DIST_DIR%\DEPLOY_HINT.txt"
echo - backend default storage.root-dir is /data/pdf-merge; ensure it is writable in your runtime.>>"%DIST_DIR%\DEPLOY_HINT.txt"

echo Done.
exit /b 0

:MODE_DEPLOY
call "%~dp0build-deploy4.bat" package
