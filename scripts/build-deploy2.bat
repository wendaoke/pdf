...
if /i "%MODE%"=="build" call :doBuild & exit /b %errorlevel%
if /i "%MODE%"=="package" call :doPackage & exit /b %errorlevel%
if /i "%MODE%"=="deploy" call :doDeploy & exit /b %errorlevel%

echo Invalid MODE: %MODE%
echo Usage: build-deploy2.bat [build^|package^|deploy]
exit /b 1
...
rem Backend jars: copy newest jar by modification time order
set "API_JAR="
...
rem Frontend standalone output (Next.js)
